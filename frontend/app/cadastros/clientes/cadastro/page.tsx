...
function CadastroClienteInner() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      try {
        const dados = await buscarCadastroPorId(supabase, 'clientes', id!);
        setNome(dados.nome || '');
  ...
  async function handleSalvar(e: React.FormEvent) {
    ...
    try {
      ...
      if (editando) {
        await atualizarCadastro(supabase, 'clientes', id!, params);
      } else {
        await criarCadastro(supabase, 'clientes', { ...params, unidade_id: unidadeId });
      }
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => router.push('/cadastros/clientes')} className="text-gray-500 hover:text-gray-700 text-xl">←</button>
        ...
        <form onSubmit={handleSalvar} className="bg-white rounded-lg shadow p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Rua, número, bairro, cidade (útil pra delivery)"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
