...
export default function ClientesPage() {
  ...
  const [paraDeletar, setParaDeletar] = useState<Cliente | null>(null);
  const [deletando, setDeletando] = useState(false);

  async function carregar() {
    setLoading(true);
    try {
      const data = await listarCadastros(supabase, 'clientes', unidadeId);
  ...
  async function confirmarExclusao() {
    ...
    setDeletando(true);
    try {
      await deletarCadastro(supabase, 'clientes', paraDeletar.id);
      setParaDeletar(null);
      await carregar();
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex-1">👥 Clientes</h1>
          <Link
            href="/cadastros/clientes/cadastro"
              ...
              <tbody className="divide-y">
                {itens.map((c) => (
                  <tr key={c.id} className={`hover:bg-gray-50 ${!c.ativo ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-4 text-sm text-gray-600">{c.telefone || '-'}</td>
                    <td className="px-4 py-4 text-sm text-gray-600">{c.email || '-'}</td>
                    <td className="px-4 py-4 text-sm">{c.ativo ? '✅ Ativo' : '⏸️ Inativo'}</td>
                    <td className="px-4 py-4">
                      <div className="flex gap-1 justify-center">
                        <Link
                          href={`/cadastros/clientes/cadastro?id=${c.id}`}
                          className="text-xs bg-yellow-100 hover:bg-yellow-200 text-yellow-700 px-2 py-1 rounded"
                        >
                          ✏️
                        </Link>
                        <button
                          onClick={() => setParaDeletar(c)}
      ...
      {paraDeletar && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold mb-2 text-red-700">🗑️ Excluir cliente?</h2>
