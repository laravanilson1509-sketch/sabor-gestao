...
export default function FuncionariosPage() {
  ...
  const [paraDeletar, setParaDeletar] = useState<Funcionario | null>(null);
  const [deletando, setDeletando] = useState(false);

  async function carregar() {
    setLoading(true);
    try {
      const data = await listarCadastros(supabase, 'funcionarios', unidadeId);
  ...
  async function confirmarExclusao() {
    ...
    setDeletando(true);
    try {
      await deletarCadastro(supabase, 'funcionarios', paraDeletar.id);
      setParaDeletar(null);
      await carregar();
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex-1">👨‍🍳 Funcionários</h1>
          <Link
            href="/cadastros/funcionarios/cadastro"
            ...
            <table className="w-full">
              <thead className="bg-gray-100 border-b">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900">Cargo</th>
              ...
              <tbody className="divide-y">
                {itens.map((f) => (
                  <tr key={f.id} className={`hover:bg-gray-50 ${!f.ativo ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-4 text-sm text-gray-600">{f.cargo || '-'}</td>
                    <td className="px-4 py-4 text-sm text-gray-600">{f.telefone || '-'}</td>
                    <td className="px-4 py-4 text-sm">{f.ativo ? '✅ Ativo' : '⏸️ Inativo'}</td>
                    <td className="px-4 py-4">
                      <div className="flex gap-1 justify-center">
                        <Link
                          href={`/cadastros/funcionarios/cadastro?id=${f.id}`}
      ...
      {paraDeletar && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-bold mb-2 text-red-700">🗑️ Excluir funcionário?</h2>
